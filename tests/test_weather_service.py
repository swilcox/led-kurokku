...
class TestWeatherService:
    def setup_method(self):
        ...
        self.location1 = WeatherLocation(name="nashville", lat=36.1627, lon=-86.7816)
        self.location2 = WeatherLocation(name="san_francisco", lat=37.7749, lon=-122.4194)
    ...
    async def test_connect_to_instance(self, mock_redis):
        ...
        mock_redis.assert_called_once_with(
            host=self.instance1.host,
            port=self.instance1.port,
            db=0
        )
    
    @pytest.mark.asyncio
    @patch("led_kurokku_cli.services.weather_service.get_temperature_data")
    @patch("led_kurokku_cli.services.weather_service.WeatherService.connect_to_instance")
    async def test_update_temperature_for_location(self, mock_connect, mock_get_temp):
        ...
        mock_get_temp.assert_called_once_with(
            self.config.openweather_api_key,
            self.location1
        )
    ...
    @patch("led_kurokku_cli.services.weather_service.get_temperature_data")
    @patch("led_kurokku_cli.services.weather_service.WeatherService.connect_to_instance")
    @patch("led_kurokku_cli.services.weather_service.WeatherService.update_brightness_settings")
    async def test_update_temperature_with_sun_data(self, mock_update_brightness, mock_connect, mock_get_temp):
    ...
    async def test_update_brightness_settings_no_change(self, mock_connect):
        ...
        sun_data = {
            "sunrise": time(6, 5),  # Only 5 minutes different
            "sunset": time(18, 5)   # Only 5 minutes different
        }
    ...
    @patch("led_kurokku_cli.services.weather_service.WeatherService.connect_to_instance")
    async def test_update_alerts_for_location(self, mock_connect, mock_process_alerts):
        """Test updating alerts for a location."""
        # Mock the alerts data
        mock_process_alerts.return_value = [
            {
        ...
        # Mock the Redis client and ensure it returns the proper async iterator
        mock_client = AsyncMock()
        # Create a mock async iterator for scan_iter
        scan_iter_mock = AsyncMock()
        scan_iter_mock.__aiter__.return_value = []
        mock_client.scan_iter.return_value = scan_iter_mock
        ...
        # Patch the scan_iter method more directly to avoid awaiting issues
        with patch.object(service, 'update_brightness_settings', AsyncMock(return_value=True)):
        ...
        # Check that connect_to_instance was called for both instances
        assert mock_connect.call_count == len(self.registry.instances)
