...
from ..models.weather import WeatherConfig, WeatherLocation, load_weather_config
...
class WeatherService:
    ...
    async def update_temperature_for_location(self, location: WeatherLocation) -> Optional[str]:
        """
        ...
        for instance in self.registry.instances:
            try:
                ...
                # Update brightness settings if sunrise/sunset data is available
                if sun_data:
                    await self.update_brightness_settings(client, sun_data)
                
                await client.close()
                
                logger.info(f"Updated temperature for {location.name} on {instance.name}")
            except Exception as e:
                logger.error(f"Error updating temperature on {instance.name}: {e}")
        
        return formatted_temp
        
    async def update_brightness_settings(self, client: redis.Redis, sun_data: Dict) -> bool:
        """
        ...
        try:
            ...
            if not config_json:
                logger.warning("No configuration found in Redis, cannot update brightness settings")
                return False
            ...
            if (current_begin_time is None or 
                current_end_time is None or
                abs((current_begin_time.hour - sun_data["sunrise"].hour) * 60 + 
                    (current_begin_time.minute - sun_data["sunrise"].minute)) >= 10 or
                abs((current_end_time.hour - sun_data["sunset"].hour) * 60 + 
                    (current_end_time.minute - sun_data["sunset"].minute)) >= 10):
                ...
                logger.info(f"Updated brightness settings: begin={new_begin}, end={new_end}")
                return True
            else:
                logger.debug("Sunrise/sunset times have not changed significantly, skipping update")
                return False
    ...
    async def update_alerts_for_location(self, location: WeatherLocation) -> List[Dict]:
        ...
        for instance in self.registry.instances:
            try:
                ...
                async for key in client.scan_iter(f"{REDIS_WEATHER_ALERT_KEY_PREFIX}{location.name}:*"):
                    await client.delete(key)
                
                # Set new alerts
                for i, alert in enumerate(alerts):
                    redis_key = f"{REDIS_WEATHER_ALERT_KEY_PREFIX}{location.name}:{i}"
                    await client.set(redis_key, json.dumps({
                        "timestamp": datetime.now().isoformat(),
                        "message": alert["message"],
                        "priority": 1,  # Higher priority than regular alerts
                        "display_duration": len(alert["message"]) * 0.4,
...
def run_weather_service(config: Optional[WeatherConfig] = None):
    """
