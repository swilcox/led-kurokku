...
REDIS_WEATHER_ALERT_KEY_PREFIX = "kurokku:weather:alert:"


async def fetch_openweather_data(api_key: str, location: WeatherLocation) -> Optional[Dict[str, Any]]:
    """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Error fetching OpenWeather data: {response.status} - {error_text}")
                    return None
...
async def get_temperature_data(api_key: str, location: WeatherLocation) -> Tuple[Optional[str], Optional[Dict], Optional[Dict]]:
    """
    ...
    try:
        ...
        if "sys" in weather_data and "sunrise" in weather_data["sys"] and "sunset" in weather_data["sys"]:
            ...
            logger.debug(f"Extracted sunrise ({sunrise_time.time()}) and sunset ({sunset_time.time()}) times for {location.name}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"Error fetching NOAA alerts: {response.status} - {error_text}")
                    return []
    except Exception as e:
